...
public class AccountTable
{
	...
	private static final String UPDATE_ACCOUNT_LOGIN_LOG = "INSERT INTO accounts_log (login, ip, last_active) VALUES (?, ?, ?)";
	...
	/**
	 * @param login : The {@link String} used as login.
	 * @param addr : The {@link String} ip address.
	 ...
	 */
	public boolean setAccountLoginLog(String login, String addr, long currentTime)
	{
		try (Connection con = ConnectionPool.getConnection();
			PreparedStatement ps = con.prepareStatement(UPDATE_ACCOUNT_LOGIN_LOG))
		{
			ps.setString(1, login);
			ps.setString(2, addr);
			ps.setLong(3, currentTime);
		...
		{
			LOGGER.error("Error in log insert {}.", e, login);
			return false;
		}
		return true;
	}
