...
public class AugmentationData implements IXmlReader
{
	...
	private final List<List<Integer>> _blueSkills = new ArrayList<>(10); // non grade
	private final List<List<Integer>> _purpleSkills = new ArrayList<>(10); // mid
	private final List<List<Integer>> _redSkills = new ArrayList<>(10); // high
	private final List<List<Integer>> _blackSkills = new ArrayList<>(10); // top
	...
	protected AugmentationData()
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			_blackSkills.add(new ArrayList<>());
		}
	...
	public void load()
	{
		...
		final int black = _blackSkills.stream().mapToInt(List::size).sum();
		LOGGER.info("Loaded {} blue (none grade), {} purple (mid grade), {} black (top grade) and {} red (high grade) Life-Stone skills.", blue, purple, black, red);
	}
		...
		forEach(doc, "list", listNode ->
		{
			forEach(listNode, "augmentation", augmentationNode ->
			{
				...
				switch (set.getString("type"))
				{
					...
					case "black" -> _blackSkills.get(k).add(augmentationId);
				}
	...
	public Augmentation generateRandomAugmentation(int lifeStoneLevel, int lifeStoneGrade)
	{
		...
		if (generateSkill)
		{
			stat34 = switch (lifeStoneGrade)
			{
				case AbstractRefinePacket.GRADE_NONE -> _blueSkills.get(lifeStoneLevel).get(Rnd.get(0, _blueSkills.get(lifeStoneLevel).size() - 1)); // blue skill
				case AbstractRefinePacket.GRADE_MID -> _purpleSkills.get(lifeStoneLevel).get(Rnd.get(0, _purpleSkills.get(lifeStoneLevel).size() - 1)); // purple skill
				case AbstractRefinePacket.GRADE_HIGH -> _redSkills.get(lifeStoneLevel).get(Rnd.get(0, _redSkills.get(lifeStoneLevel).size() - 1)); // red skill
				case AbstractRefinePacket.GRADE_TOP -> _blackSkills.get(lifeStoneLevel).get(Rnd.get(0, _blackSkills.get(lifeStoneLevel).size() - 1)); // black skill
				default -> stat34;
			};
