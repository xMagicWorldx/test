	...
	public boolean useVoicedCommand(String command, Player player, String target)
	{
		if (command.startsWith(VOICED_COMMANDS[0]))
		{
			...
			if (player.getPremiumService() == 0)
			{
				...
				htm.setFile("./data/html/mods/menu/nopremium.htm");
			...
			else
			{
				...
				NpcHtmlMessage htm = new NpcHtmlMessage(0);
				htm.setFile("./data/html/mods/menu/premium.htm");

				htm.replace("%rate_xp%", "" + Config.PREMIUM_RATE_XP);
				htm.replace("%rate_sp%", "" + Config.PREMIUM_RATE_SP);
				htm.replace("%rate_drop%", "" + Config.PREMIUM_RATE_DROP_ITEMS);
				htm.replace("%rate_spoil%", "" + Config.PREMIUM_RATE_DROP_SPOIL);
				htm.replace("%expires%", "" + String.valueOf(format.format(end_prem_date)));
				player.sendPacket(htm);
